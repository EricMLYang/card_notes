...
## A. 主脈絡與個人映射
- 論證骨架：作者用「創投 × PM × 開發者」三視角，把 Vertical AI 的產品化拆成七部分——為什麼 Vertical、產品定位（Horizontal/Vertical/Orthogonal × SaaS-first/AI-first × 工具/服務/成果）、Databricks 三層架構（數據→策略→互動）、F.D.A. 找 Agent 題目法、Agent vs Workflow vs RAG 選型、Agent 七型態、技術棧四大組件（Brain/Memory/Hands/Orchestration），最後落到實戰兩案例（AWS 行銷 Agent、Databricks 分店診斷 Agent）與九大踩坑。
- 挑戰的預設：「商業模式從 SaaS 直接跳到 RaaS」「Agent 越自動越好」「向量 DB = 長期記憶」「multi-agent 協作 = 更強」「prompt 多寫一些就行」。
- 個人映射：是 D2D Architect / RMN Vertical AI 主軸的「整本工作說明書」——直接對應到「在 Databricks + 應用層 in 特定領域」這個架構，可作為團隊內訓 + 對外提案的共用框架；尤其「F.D.A. 找題目法」「Agent 化 5 維判定」「9 大實戰踩坑」是高密度可重用素材。

## B. 候選卡（Lite）

序號 1
- 候選標題：技術護城河五階梯 — 從 Prompt → 靜態 RAG → CoT 邏輯 → Fine-tuning → 資料飛輪 + Agent
- 分級：Core
- 類型：Pattern
- 核心內容：AI 產品技術競爭力分五層：(1) 特定 AI 能力（基於 Prompt，最易複製）；(2) 靜態資料 RAG（資料可被購買就失守）；(3) 領域 Know-how（CoT 邏輯，可被觀察複製）；(4) 過程數據 SFT/RLHF（從用戶修正中生長，難複製）；(5) 深度整合 + 資料飛輪（Function Calling + Agentic Workflow + 即時運行的生態系，要複製必須複製整套硬體與多年運行數據）。Vertical AI 的價值是把這五層在特定場景「極端化」。
- 保留理由：是回答「我們的護城河在哪一層」的決策框架，可直接對 RMN 產品做評估。
- 待補強處：第五層的「最小可行飛輪」設計（最早期怎麼啟動？資料量多少才有意義？）。
- 初步知識鉤子：Data Flywheel、Domain-specific Embedding、Telemetry as Training Data、Model Distillation。

序號 2
- 候選標題：產品定位三軸 — 市場（Horizontal/Vertical/Orthogonal）× 賣什麼（工具/服務/成果）× 邏輯（SaaS-first/AI-first）
- 分級：Core
- 類型：Heuristic
- 核心內容：AI 產品定位必須回答三個正交問題：(1) 切哪種市場——跨產業（Horizontal）/ 特定產業（Vertical）/ 跨產業同職能（Orthogonal）；(2) 賣什麼——更好的工具（SaaS+AI）/ 直接服務（Vertical AI）/ 賣成果（RaaS）；(3) 設計邏輯——流程中心（SaaS-first，人是司機）/ 成果中心（AI-first，人是主管）。三軸的選擇決定技術架構、商業模式、團隊組成。
- 保留理由：是 Vertical AI 提案 / 對外溝通的核心話語結構。
- 待補強處：三軸的相容組合與不可能組合（哪些組合會自相矛盾？）。
- 初步知識鉤子：Salesforce + Agentforce 多重定位、Cursor 正交 AI、簡立峰「軟體是工具、服務是目標」。

序號 3
- 候選標題：F.D.A. 框架 — 找高價值 Agent 題目的三個訊號（Friction × Data Nexus × Actionable Outcome）
- 分級：Core
- 類型：Heuristic
- 核心內容：F.D.A. 三維篩選：(1) Friction——高頻、重複、需人工審核的環節（如素材合規檢查）；(2) Data Nexus——非結構化數據的交匯點（人流影像 + POS 銷售明細），「交叉後的決策」最值錢；(3) Actionable Outcome——能否從「給圖表」進化到「交付工作成果」（自動結案報告）。三個都打勾的題目才值得做 Agent。
- 保留理由：是把抽象的「該做哪個 Agent」變成可逐題打分的工具，直接落地。
- 待補強處：三維是否權重相同？或某維度（如 Actionable Outcome）是否一票否決？
- 初步知識鉤子：Selling the Work、Outcome-based Pricing、零售多感測器融合。

序號 4
- 候選標題：Agent 化適配的 6 維 Scorecard — 規則明確 / 可重複 / 可驗證 / 資料可得 / KPI 可量 / 風險可控
- 分級：Core
- 類型：Pattern
- 核心內容：判斷一個任務值不值得開發 Agent 的六維 scorecard：(1) Task Definition Clarity；(2) Repetitiveness/Frequency；(3) Outcome Verifiability；(4) Data Availability & Quality；(5) Objective Metrics；(6) Risk/Safety Constraints。高優先 = 高重複 + 明確判準 + 大量資料 + feedback loop + 低中風險。次優先（需 HITL）= 創意策略 / 社會情境判斷 / 高風險金融醫療。
- 保留理由：把 F.D.A. 的「找題目」延伸到「該做到多自動」的決策。
- 待補強處：六維評分的權重設計（可否依產業調整？）。
- 初步知識鉤子：F.D.A. 框架、Anthropic Agent 7 型態、Polanyi's Paradox。

序號 5
- 候選標題：類程式開發的高 Agent 化領域判定 5 維 — 可模擬 / 回饋低延遲 / 規則可形式化 / 可驗證閉環 / 高品質資料
- 分級：Core
- 類型：Heuristic
- 核心內容：要評估一個領域能否「像程式開發那樣」被 Agent 高效自動化，從五維判斷：(1) 狀態空間可模擬性（沙盒可重現？）；(2) 回饋延遲（編譯秒級 vs 藥物實驗月級）；(3) 規則與約束可形式化（DSL/SQL/狀態機）；(4) 自動驗證器存在 + 失敗可定位（compiler/unit test 等價物）；(5) 高品質 input-output 配對資料。程式開發在五維都優異——這是它成為 AI 戰場的根因。其他領域要對標時，這五維就是診斷工具。
- 保留理由：是「為什麼 SWE Agent 領先、其他領域跟不上」的結構性答案；對 Vertical AI 評估產業可行性極關鍵。
- 待補強處：每個維度的「最低門檻」量化指標。
- 初步知識鉤子：SWE-Bench Verified、Self-Improvement (TTRL/R-Zero)、Domain-specific RL。

序號 6
- 候選標題：Agent 七型態分類 — Business-task / Conversational / Research / Analytics / Developer / Domain-specific / Browser-using
- 分級：Support
- 類型：Pattern
- 核心內容：常見 Agent 形態分七類：(1) Business-task（流程自動化）；(2) Conversational（聊天入口）；(3) Research（找資料 + 引用可追溯）；(4) Analytics（自然語言查數倉，特別適合 Databricks + 應用層）；(5) Developer（IDE/Repo 工具，seat 收費）；(6) Domain-specific（最接近 Vertical AI，封裝專業流程）；(7) Browser-using（GUI 操作 + RPA 升級）。每類有不同 ROI 模型與商業化路徑。
- 保留理由：產品定位 / 內訓 / 對外提案的分類詞彙。
- 待補強處：類別之間的混合形態（Analytics + Domain-specific 是 Vertical Data Agent）。
- 初步知識鉤子：Vertical AI 案例譜、Analytics Agent vs BI Tool、Browser Agent 風險。

序號 7
- 候選標題：Outcome-based Pricing 不是趨勢全勝 — 採用率 < 15%、Salesforce 退回 seat+usage 混合
- 分級：Core
- 類型：Warning
- 核心內容：客戶想買成果，但更想要「成本可預測性」。24–25 年期間 < 15% AI-native 公司採用 outcome pricing，多在實驗階段。Salesforce Agentforce 推出沒多久就從 per-conversation 退回 seat+usage 混合。主要風險：歸因爭議（Zendesk：bot 回覆後客戶又回，算 resolved 嗎？）、會計複雜（ASC 606 區分 access vs delivery）、客戶承受度（70% 高管因成本取消或延遲 GenAI 專案）。Burstable reserve（基準價 + 超量按 outcome）是當前最務實的混合解。
- 保留理由：直接打掉「outcome pricing 是必然趨勢」的常見論述；對 RMN 產品定價設計關鍵。
- 待補強處：哪些垂直領域 outcome pricing 已被驗證可行（Intercom Fin / Sierra / Decagon）。
- 初步知識鉤子：RaaS 設計、ASC 606、Financial Certainty vs Innovation Tradeoff。

序號 8
- 候選標題：95% Production 失敗主因 — 不是模型不夠強，是上下文選擇（Context Selection）系統做不好
- 分級：Core
- 類型：Warning
- 核心內容：Flask 作者 Armin 等人的踩坑共識：Agent 進 production 約 95% 失敗，主因不是模型，而是「周邊工程」——context engineering / memory / error handling / RAG / evals / feedback loop / 成本與路由。多數團隊以為在做 AI，其實在做 context selection。RAG 常做得太天真；Text-to-SQL 殘酷因語意歧義；信任是人的問題（成功 5% 通常是人機協作而非全自動）。
- 保留理由：是把「失敗模式」說清楚的權威性論述；可作為產品 review 的失敗清單。
- 待補強處：失敗的詳細分類與比例（哪一項佔多少？）。
- 初步知識鉤子：Context Engineering、Repo-as-Worker、Surge AI 能力金字塔。

序號 9
- 候選標題：Opinionated AI — 彈性是陷阱，產品要替用戶做前置決策
- 分級：Core
- 類型：Heuristic
- 核心內容：Surge AI 結論：用戶不想調溫度、調分塊策略，他們想要「結果」。每個必填選項都代表你沒替用戶做決定。產品策略：用真實場景測模型（別只信 benchmark）、prompt 清楚定義「成功長怎樣 + 怎麼達成」、Anthropic 走「領域 + 框架優化」、Claude Code 內建很多決策。「從深且窄開始」——先做能產生最大價值的 10% 任務，窄到能徹底優化、深到值得投資。
- 保留理由：把產品設計哲學從「給用戶選項」轉到「替用戶承擔判斷」，對 Vertical AI 特別關鍵。
- 待補強處：何時該保留少量彈性（advanced user 出口）。
- 初步知識鉤子：harness is key、品味作為降噪器、Selling the Work。

序號 10
- 候選標題：Databricks 三層架構心智 — 基礎系統（Rules→Data） / 策略分析（Data→Insight） / 互動 AI（Insight→Action）
- 分級：Support
- 類型：Pattern
- 核心內容：用 Databricks 為產品核心的三層思維：(1) 基礎系統層——業務系統把 Rules 沉澱成 Data，決定「能回答哪些問題」；(2) 策略分析層——Pipeline + Business Data Science 從數據萃取意義，決定「回答多有洞見」；(3) 互動 AI 層——GenAI 提升理解、Agent 接到流程動作，決定「價值能否被用戶接收」。三層共同決定最終價值，缺一不可。對應的 Pipeline → GenAI → Agent 也是「系統與人的關係」的演進（聽話→說話→對話）。
- 保留理由：是 RMN / Vertical AI 對外解釋「Databricks 怎麼長出產品」的核心話語結構。
- 待補強處：三層之間的反饋迴路（Action → Data 怎麼回流？）。
- 初步知識鉤子：Medallion Architecture、Decision Loop、Pipeline-to-Agent Evolution。

## C. 建議送 refine 的項目
- 序號 1, 2, 3, 4, 5（最高優先：是 Vertical AI 主軸的五張支柱卡，建議全部進 refine）
- 序號 7, 8, 9（最高優先：失敗模式 + 反共識 + 產品哲學三張，與支柱卡並列）
- 序號 6, 10（中優先：分類 / 架構心智，可作為前述卡片的橋接卡）
- 註：本檔內容極密，refine 階段可考慮再展開（特別是「9 大實戰踩坑」與「兩個 case study」可分別生新卡）

## D. 呼叫 refine-cards
- 將上述 10 張候選卡交由 refine-cards 精煉。本檔為 high-density Vertical AI 主檔，建議 refine 後考慮分批落正式卡（避免一次卡量過大）。

> Stage 1 (Vertical AI 主題) 已提取 1 卡 (序號 7 → 7-005_Outcome-based_Pricing)，其餘候選卡待 Stage 2