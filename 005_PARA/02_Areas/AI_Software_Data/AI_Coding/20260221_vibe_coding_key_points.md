---
pipeline_stage: "DONE"
topic: "AI 24/7 運作的關鍵控制點"
scenario: "opinion"
selected_cards: []
chosen_title: "AI 24/7 運作的關鍵控制點：以判斷權角色重新分類"
chosen_hook: "少了一個根本的問題：這些控制點，到底該由誰來判斷？"
...
「設品質門檻」聽起來很合理，但誰決定門檻要多嚴？「最小權限」大家都知道，但誰來劃那條線？清單上每一條都正確，問題是當它們全部攤在桌上，沒有人知道哪個是自己的事。

所以我換了一個角度：按「**誰擁有這個判斷權**」重新歸類。

一旦判斷權歸屬明確，控制點就不再是一張人人都覺得重要、但沒人真正負責的待辦清單。它變成五個角色各自的職責邊界——PM 管目的、架構師管結構、開發管實作、QE 管可信度、維運管風險。
...
## A. 主脈絡與個人映射
- 論證骨架：把 AI 24/7 運作的 10 個控制點，從「待辦清單」重新歸類到 PM／架構師／開發／QE／維運五個判斷權角色，再用 TOC 視角找出三個「沒有就絕對不能上線」的最小可行控制集（Quality Gate、隔離與最小權限、可執行需求定義）。
- 作者挑戰的預設：AI 治理清單越完整越好。實際上清單沒有判斷權歸屬就是「人人都覺得重要、沒人負責」的紙上規則。
- 個人映射：強化「judgement ownership」這一條主軸——AI 自動化的治理重點不是補功能，而是補判斷權的歸屬與最小可行控制集，可直接連到 DecisionOps / 決策權架構主題。

## B. 候選卡（Lite）

序號 1
- 候選標題：AI 治理清單沒有「判斷權歸屬」就會變成沒人負責的紙
- 分級：Core
- 類型：Principle
- 核心內容：常見的 AI 24/7 控制點清單（10 點、20 點）每一條都對，但攤開沒有人知道哪個是自己的事。控制點要可運作，必須先回答「這個判斷由誰擁有」。把每個控制點掛到 PM／架構師／開發／QE／維運五個角色之一，責任邊界才會清楚。
- 保留理由：抓出治理清單的根本病灶（責任真空），是高遷移性的判斷原則。
- 待補強處：跨角色控制點（如變更管理）的拆解規則待補。
- 初步知識鉤子：DecisionOps、判斷權架構、責任分配矩陣（RACI）、Harness Engineering。

序號 2
- 候選標題：AI 24/7 的最小可行控制集只有三個：Gate、Sandbox、可執行需求
- 分級：Core
- 類型：Heuristic
- 核心內容：用 TOC 視角篩控制點：① Quality Gate（QE）擋掉不該進 production 的東西；② 最小權限與隔離（架構師）讓出錯爆炸半徑可控；③ 可執行的需求定義（PM）避免 AI 高效做錯事。其他 7 個控制點都重要，但這三個是地基，沒打好其他都是空轉。
- 保留理由：把 AI 治理從清單收束到三件事，符合「地基打好再擴張」的取捨邏輯。
- 待補強處：三件事的最小實作門檻、不同團隊規模的調整方式可再具體化。
- 初步知識鉤子：限制理論（TOC）、AI 24/7 啟動三階段、最小可行護欄、SCAR 提案敘事。

序號 3
- 候選標題：開發者核心能力從 writing 轉向 reviewing
- 分級：Support
- 類型：Pattern
- 核心內容：當 AI 寫 80% 初稿，開發者最關鍵的能力從「會寫」變成「能判斷 AI 寫的能不能上線」。要看的有三件事：結構性審查（架構意圖）、品味審查（命名與可維護性）、風險直覺（感覺不對但說不出為什麼）。這三類審查 AI review 工具抓不到，是人類最後的判斷護欄。
- 保留理由：把「角色重定義」具體化到三個可訓練的能力，比泛泛的「轉型」有操作性。
- 待補強處：如何系統化訓練「風險直覺」未交代。
- 初步知識鉤子：IC 到判斷者的角色遷移、Code Review 的人類獨有能力、Kent Beck AI 工程角色重估。

序號 4
- 候選標題：AI 監控指標必須伴隨「踩煞車閾值」才算控制機制
- 分級：Support
- 類型：Warning
- 核心內容：AI PR 數量、Bug 率、Lead Time 等指標如果沒有定義「什麼數字出現要暫停 AI 自主提交、什麼數字要縮小 AI 任務範圍」，監控就只是儀表板，不是控制機制。QE 必須事先約定踩煞車條件，事後才有判斷基準。
- 保留理由：補上「指標 vs 控制」的常見混淆，可遷移到 data product 監控設計。
- 待補強處：閾值設計範本（百分比、滑動窗口）尚未提供。
- 初步知識鉤子：可觀測性 vs 控制機制、SLO/SLI、AI workflow guardrails、Decision Loop。

序號 5
- 候選標題：事故根因要按「誰的判斷失守」回溯，不能只壓給 reviewer
- 分級：Support
- 類型：Heuristic
- 核心內容：AI 變更出事時，傳統把責任壓在「相關模塊的人類工程師」太籠統。應按判斷權回溯：需求不清→PM、架構漏洞→架構師、審查疏忽→開發、Gate 不夠嚴→QE、流程設計缺陷→維運。這樣每次事故都能精準改善該層的判斷標準，而不是用「以後要更小心」收場。
- 保留理由：把「責任歸屬」從口號變成可操作的回溯路徑，配合序號 1 形成完整框架。
- 待補強處：跨角色事故的歸因比例如何分配。
- 初步知識鉤子：事後復盤（Postmortem）、Decision Audit、AI 治理閉環。

## C. 建議送 refine 的項目
- 序號 1（Core Principle）
- 序號 2（Core Heuristic，與序號 1 互補形成主軸）
- 序號 3（Support，AI 角色遷移橋接卡）
- 序號 4（Support，補強指標 vs 控制機制混淆）
- 序號 5（Support，補完責任歸屬回溯）
