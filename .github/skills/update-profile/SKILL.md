...
## 目標
接收用戶提供的喜歡文章，分析其共通點，萃取出精煉的閱讀偏好設定，寫入 `000_MyContext/capture_profile.md` 的「閱讀偏好」區段，讓 scout-news 等 skill 能根據偏好更精準地搜尋和篩選內容。
...
【檢查點】用戶確認/調整分析結果
         ↓
[Step 3] UPDATE    寫入 capture_profile.md
         ↓
輸出更新摘要
```
...
- **貼上全文**：用戶直接貼文章內容
- **指向本地檔案**：指定 `015_Write/Publish/` 或 `003_Capture/` 中的檔案路徑
...
### Step 3: UPDATE（更新 capture_profile）
**模式**：全自動

1. 讀取 `000_MyContext/capture_profile.md`
2. 更新以下區段：
   - `> 最後更新：` 改為今天日期
...
```markdown
## capture_profile 更新摘要

- 分析文章數：X 篇
- 更新日期：YYYY-MM-DD
- 更新檔案：`000_MyContext/capture_profile.md`
...
**與其他 skill 的串接**：
- `update-profile` → 更新 `capture_profile.md` → `scout-news` 讀取偏好搜尋更精準的新聞

## 禁止事項

- ❌ 不要在用戶確認前就寫入 capture_profile.md
- ❌ 不要修改「關注方向清單」區段（那是用戶手動維護的）
- ❌ 不要用模糊描述（如「有趣的主題」），要精確到可當搜尋指令
- ❌ 不要過度推論——只從提供的文章中萃取，不加入文章沒有的偏好
- ❌ 不要刪除 capture_profile.md 中已有的其他內容
