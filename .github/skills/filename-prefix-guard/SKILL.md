...
description: 統一檢查與修正 000Inbox、004_1Draft、004_2Express 的檔名日期前綴規則。適用於用戶說「檢查檔名格式」「補上日期前綴」「rename files with date prefix」時。
---
...
## 觸發條件
- 用戶說「幫我檢查 Inbox/Draft/Express 檔名」
- 用戶說「補上日期前綴」
- 用戶說「rename files with date prefix」
- 用戶說「檢查檔名格式」

## 禁止事項
- ❌ 不要修改 `/003_2Cards/` 等非目標資料夾
- ❌ 不要移除原檔名語意，只能加上日期前綴
- ❌ 不要跳過日期有效性檢查
