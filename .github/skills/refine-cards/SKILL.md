...
- 範疇：AI 如何進入真實業務、產品、流程並產生價值（AI 功能嵌入產品、agent productization、workflow design、商業價值）
...
- 範疇：AI 如何直接參與開發與知識工作（Copilot、agent skills、repo context、agentic workflow）
...
- 範疇：資料工程、分析建模、BI 與決策支持（ETL/ELT、數據管線、指標體系、Business Data Science、decision system、因果推論、語意層）
...
- 範疇：Databricks 平台專屬知識（Unity Catalog、Delta Lake、DLT、Mosaic AI、Lakehouse 應用、semantic layer、governance、deployment）
...
- 範疇：測試、維運、監控、穩定性（testing、observability、monitoring、evaluation、validation、guardrails、deployment responsibility）
...
4. **若是資料、語意、指標、因果、決策支持、decision loop**：優先 `5-`
5. **若是 data product、vertical AI、workflow productization、產品化判準**：優先 `7-` 或 `3-`，以「最常支撐哪種決策」判斷
6. **非特定技術域但可跨域重用的思維框架**：歸 `1-`
7. **個人與知識系統**：
   - 知識如何被組織與管理 → `2-`
   - 人如何成長與執行 → `12-`
   - 如何表達與溝通 → `13-`
...
#### 7.1 寫正式卡
- 目錄：`010_CardNotes/02_Cards/{分類資料夾}/`（根據索引對應的分類資料夾，如 `03_ai_applications_and_productization/`、`01_mental_models/` 等）
- 檔名格式：`{分類前綴}{流水號}_{卡片標題}.md`
- 範例：`010_CardNotes/02_Cards/03_ai_applications_and_productization/3-001_AI產品化的核心挑戰.md`
