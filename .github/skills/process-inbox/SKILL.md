...
2. 呼叫 `break-cards` skill 執行拆解，產出：
   - 主脈絡（論證骨架）
   - 候選卡片清單（每張含：標題、類型、概念、重要性、邊界/反例、知識鉤子）
   - 連結建議
3. 根據卡片內容，自動判斷每張卡片的**編號分類**（遵循 `.github/skills/break-cards/SKILL.md`「編號分類對照表」，含判斷順序與衝突處理規則）
   - 一篇文章可能產生多個不同類別的卡片
...
> 卡片內容欄位規格以 `break-cards` 為準；若欄位定義有差異，統一採用 `break-cards`。
...
## 類型
[Principle / Heuristic / Model / Stance / Warning / Pattern / Leverage / Trend / Question]

## 概念
[50-300 字的完整描述，含機制與底層邏輯]
...
## 知識鉤子
[以 1-2 句說明具體連結方向，例如可作為哪個概念的底層實踐，或與哪個概念形成對比]
...
> 歸檔決策與執行統一呼叫 `archive-article` skill（用這新 skills 歸檔）。
...
| 情境 | 處理方式 |
|------|----------|
| `break-cards` 拆解後無卡片產出 | 告知用戶「此文章未發現符合高訊號標準的卡片」，建議歸檔到 `/005_1Resource/` 或 `/006Archive/`，跳過 Step 4-6 |
| 用戶在檢查點刪除全部卡片 | 確認是否仍要歸檔原文，跳過 Step 4-6，直接進入 Step 7 |
| 卡片檔名與 `/003_2Cards/` 中已存在的卡片重名 | 在標題後加上區分後綴（如 `3-AI Agent 的本質-v2.md`），提示用戶確認 |
| 索引檔中找不到合適的 `##` 段落 | 新增一個 `##` 段落，段落名用卡片主題關鍵字命名 |
...
- ✅ 每張卡片都有完整的內容（類型、概念、重要性、邊界/反例、知識鉤子）
- ✅ 對應的索引檔都已更新
- ✅ 原文已歸檔到用戶確認的位置
- ✅ Inbox 中已處理的檔案已被移走
