...
An LLM call to pick which of the available indexes to use, given the user-supplied query and the developer-supplied description of the indexes.

A retrieval step that queries the chosen index for the most relevant documents for the user query.

Another LLM call to generate an answer, given the user-supplied query and the list of relevant documents fetched from the index.
...
Let’s take as an example a text-to-SQL application, which receives as input from the user a natural language description of some calculation to make over a database. We mentioned earlier that this could be achieved with a single LLM call, to generate a SQL query, but we can create a more sophisticated application by making use of multiple LLM calls in sequence. Some authors call this architecture flow engineering.2

First let’s describe the flow in words:

One LLM call to generate a SQL query from the natural language query, provided by the user, and a description of the database contents, provided by the developer.

Another LLM call to write an explanation of the query appropriate for a nontechnical user, given the query generated in the previous call. This one could then be used to enable the user to check if the generated query matches his request.
...
這章在回答一個核心問題：**Prompt、RAG、Memory 都學過了，那要怎麼「組裝」成一個能達成目標的完整應用？**
作者用「建築」做比喻：材料一樣，但**架構（architecture）**決定成品是游泳池還是房子。接著引入一個關鍵張力：**自治能力（agency）越高越省人力，但越容易出錯；可靠性（reliability/oversight）越強越安全，但自治就受限。**
最後用 LangGraph 把常見做法整理成一組「配方」：**LLM Call → Chain → Router**（更 agentic 的架構留到後面章節）。

---

## 你可能會想記錄的重點（依每節整理）

### 1) 為什麼要談「Cognitive Architectures」

* LLM 應用不缺元件（prompt/RAG/memory），缺的是**把元件串成可預期行為的組合方式**。
* **Cognitive architecture** = 一種「配方」：定義整個應用會跑哪些 step、順序怎麼走、誰（LLM 或程式）決定下一步。

### 2) 從「目的 + 約束」開始設計（email assistant 例子）

* 所有 workflow/agent 的起點不是技術，而是：

  * **Purpose（目的）**：這個 app 要幫你做到什麼？
  * **Constraints（約束）**：它做事要被哪些規則綁住？（例如少打擾你、不要亂回信）
* 約束越清楚，越容易挑對架構與控制點。

### 3) 本章最重要的 trade-off：Agency vs Reliability

* **自治越高**：更省人工、更像助理，但更需要控制、審核與防呆。
* **可靠越高**：更可控、更像工具，但需要你介入更多。
* 這是你在設計每個 step 時，最常要做的取捨。

### 4) 用「自治光譜」描述：到底讓 LLM 決定到哪一層？

把 LLM 的「決策權」分三層（越往下越自治）：

* **決定輸出**：LLM 只負責產出內容（例如草擬回信、摘要）。
* **決定下一步**：LLM 在多個既定步驟中選擇要走哪條路（路由）。
* **決定可用步驟**：LLM 甚至能創造/改寫行為（例如動態寫程式或工具使用）——這就更接近後面章節的 agentic 架構。

### 5) 架構總覽（本章涵蓋到 Router）

作者把常見 recipe 分類（可當你設計 workflow 的「選型表」）：
...
* **用途**：單點能力（翻譯、摘要、單次生成 SQL…）。
* **本質**：LLM 是一個「函數」，輸入→輸出，流程最簡。

#### 2: Chain（固定多步）

* **用途**：需要「分工」的多步流程（先產 SQL，再解釋 SQL；甚至再執行、再總結）。
* **本質**：步驟順序由開發者固定，LLM 只在每個 step 產出結果。
* **關鍵詞**：有些人稱這種做法是 **flow engineering**（把任務拆步、每步不同 prompt/模型/溫度）。

#### 3: Router（LLM 決定走哪條路）

* **用途**：同一個入口，可能要進不同資料域/工具域（例如多個向量索引、多個資料庫、多種工具）。
* **本質**：LLM 變成「分類器/決策器」：先判斷 domain → 再走對應 retrieval/tool → 再生成答案。
* **價值**：避免把不相關資訊塞進 prompt，提升品質與效率。

### 6) LangGraph 在本章扮演什麼角色（你做 workflow/agent 會用到的核心概念）

> 你可以把 LangGraph 當成：**用「圖（graph）」把狀態（state）跟步驟（nodes）管理起來的 workflow 引擎**。

你會想記的 LangGraph 概念點：

* **State（狀態）**：整個流程共享的資料容器（例如 messages、user_query、sql_query、documents…）。
* **Reducer / 更新規則（messages 的 append）**：決定 state 怎麼被更新（例如訊息是「追加」不是覆蓋）。
* **Node（節點）**：一個 step（LLM call、retrieval、轉換、決策）。
* **Edge（邊）**：流程怎麼走（固定邊或條件邊）。
* **Conditional Edge（條件邊）**：Router 的核心，用函數把「LLM 選到的 domain」映射到下一個 node。
* **compile()**：把 builder 變成可執行的 graph。
* **stream()**：逐步吐出每個 node 的輸出（對除錯、觀察流程非常有用）。
* **可視化**：把 graph 畫出來（mermaid/png），用來溝通架構與審查流程。

---

## 這章對「Agent / 工作流怎麼建」的本質結論（你要的那種）

* **先定目的與約束，再選架構**：不是先決定「我要做 agent」，而是看你要多自治、要多可靠。
* **把 LLM 的決策權切小**：

  * 先從 LLM Call（單點能力）
  * 再到 Chain（固定多步、可控）
  * 再到 Router（LLM 只負責選路，行為仍是預先定義）
* **LangGraph 的價值**：讓你把 workflow 變成「可視化、可追蹤、可 streaming debug」的系統，而不是一坨 if-else。
* **Router 是走向 Agent 的門口**：它已經開始讓 LLM 決定「下一步」，但仍保留開發者對「可用步驟集合」的控制（比較安全）。

如果你下一章要進入真正的 agent 架構，我可以用同樣方式幫你做：**一頁選型（何時用 agent）、一頁控制點（怎麼把風險關起來）、一頁 LangGraph 對應實作骨架**。
