...
ChatGPT 3.5 剛出來，LLM 還很早期，當時公司內部一個 MI 2.0 的專案，主要是將很貴的研調數據，加上我們自己爬蟲總經數據和電商資訊，進行一些分析和報表建構



當時的聖杯是想預測面板價格，但是這實在太難了，因此轉往分析搭配新聞或報告資訊來做分析，



因此用產業分析這個詞也不算對，因為我們還是比較專注最新動態，



，主要是以面板價格預測的專案為主軸，去延伸出，用新聞和產業報告來彙整內部長官看的相關分析，偏面板下游產業 (電視、筆電、車用...等），





當時用 AI 的方法比較炫技一點，畢竟這工具剛出來，一直在想怎麼用比較好，現在看起來應該更專注在分析面
...
**我們替TV/NB/DT/中國車用 BU 處理不少研調、爬蟲數據，做了很多分析預測**

**但…只處理數字的話很有侷限**

**變化劇烈時，數據更新太慢，BU 需要更快的資訊更新   ( EX : 新聞 )**

因此希望將新聞資訊萃取出來，

- **8月 CCI 上升了，會提前嗎?**

- **9月 US GDP 下修幅度大，會延後嗎?**

- **終端價格大幅降價了，會提前嗎?**



配合以往數據分析所建立的因果關係 (EX: 庫存影響價格…），最未來推論
...
我們從議題出發，相關部門列出我們關係面板下游的、電視、筆電、電腦螢幕、車用…等議題，例如：
...
- **成　　本： 　供應商議價能力**… 等列了大概 10 個比較大方向的議題



每個新聞我們會先過一次 GPT 萃取出重要議題，



當符合有重要議題後，我們會過第2道GPT, 將裡面的重要元素和關係抓出來，很像命名實體的做法，例如： 戴爾取消美光記憶體下單，會變成，戴爾  下單  美光;  美光 生產 記憶體 …等，這些元素跟關係我們會存到 DB，當作未來抓對應數據的重要元素



  分析架構：

這張圖描述了 **AI 在數據分析專案中的應用流程**，著重於「先折解流程」的概念。整體脈絡如下：
...
1. **數據 → 圖表**
   ...
   - 經由基本統計分析與視覺化，將原始數據轉為可觀察的圖表。

2. **資訊–現象（xx 趨勢上升）**

   - 從圖表中觀察趨勢或變化，找出值得注意的現象。

3. **資訊–關係（相關 → 因果）**

   - 進一步探討現象間的關聯性與可能的因果關係。

4. **分析架構（5力、策略3C…）**

   - 套用分析模型或策略架構，形成可解釋的邏輯推論。

5. **情境論述（What if）**

   - 進行情境模擬與假設分析，用於預測或模擬未來情境。

6. **敘述分析（Story）**

   - 將分析結果轉化為故事化的敘述，協助決策者理解。
...
- 「What if」階段屬於 **預測／模擬**，而後續的數據監控屬於 **驗證／更新**。

- 最終的分析成果以「故事化敘事」支援決策制定。

---

這是一個從數據 → 資訊 → 分析 → 敘事 → 決策 的完整 AI 數據分析應用流程。





這張圖展示了 **AUO PM 主導的專案分析與報告撰寫流程**，從議題設定到審核完成的整體分工架構。流程說明如下：
...
1. **議題／目標設定（AUO PM）**
   ...
   - 由 AUO 專案經理（PM）提出分析議題或目標，啟動整個分析流程。

2. **收集資訊階段**

   - 由多位不同領域的專家共同蒐集並彙整資料，包括：
   ...
   - 這一階段的目的是整合跨領域資訊，作為後續分析的基礎。

3. **商業產品分析（分析師）**

   - 分析師根據專家提供的資訊，進行整體的商業與產品分析，萃取洞見與結論。

4. **撰寫報告（作家）**

   - 將分析結果轉化為文字與報告內容，確保邏輯清晰、表達精準。

5. **審閱（查核員）**

   - 最後由查核員審閱報告內容，確認資料正確性與邏輯一致性，完成正式輸出。

---

### 🔹整體概念

此流程強調從 **「議題 → 資料 → 分析 → 敘事 → 審核」** 的完整閉環，並透過跨領域專家協作確保報告的**深度、準確度與決策參考價值**。





這張圖展示了 **全新的 AI 輔助資料處理 Pipeline（NEW）**，主要用於自動化「議題分析與內容抽取」的流程。以下是重點說明：
...
2. **摘要 (summary)**
   ...
   - 透過 LLM（如 ChatGPT）進行摘要生成。

   - 結果儲存為「summary table」，作為後續議題辨識的基礎。

3. **議題擷取與儲存**

   - 系統從摘要中提取議題主題，並寫入「議題 Table」。
...
5. **元素分析**
   ...
   - 從消息中提取關鍵元素（如公司、事件、數據、趨勢等），建立「元素 Table」。

6. **元素連結**

   - 利用 LLM 將相同或相似的元素自動關聯整合成完整脈絡。
...
- **消息檢查重複**：\
   使用「近期資料 + Embedding 相似度」來判斷是否為重複消息。

- **元素檢查重複**：\
   以「初始資料 + Embedding 相似度」檢查相似元素，避免重複紀錄。
...
- 以 Embedding 技術確保內容不重複。

- 最終輸出 HTML 形式的分析結果或報告。

---

✅ 簡而言之，這是一條 **以 AI 為核心、結合 Embedding 相似度與知識鏈結的智能化議題分析流程**。

這張圖展示了 **AI 在文本資訊中自動抽取關鍵關係（Entity Relationship Extraction）** 的過程與成果。
...
1. **拆解元素**：從文章中辨識出關鍵實體（如公司、年份、事件等）。

2. **建立連結**：找出實體間的關係（例如「合作」、「投資」、「推出」等）。

3. **萃取資訊**：整理出具體資訊（如時間、地點、事件描述等）。
...
> 「汽車換電站將成趨勢？ Stellantis 與 Ample 合作於 2024 年啟用換電站」

AI 系統從這段文字中抽取出以下關鍵資訊：
...
這張圖代表系統能將自然語言轉換為結構化知識（Knowledge Graph），\
幫助使用者快速理解文本中的「誰、做了什麼、什麼時候」。
...
這個專案名義上雖是產業分析，但核心更偏向動態追蹤， 它始終圍繞著面板價格預測這個主軸， 並延伸至對下游產業，如電視、筆電、車用市場的觀察。
...
BU 需要更快的資訊來應對變局， 例如，即時的新聞。

像是「8 月的 CCI（消費者信心指數）上升了，市場會提前復甦嗎？」 或是「9 月美國 GDP 下修幅度很大，復甦會不會因此延後？」 甚至，「終端產品價格大幅跳水了，這會不會刺激需求提前湧現？」
...
這些被萃取出來的元素與關係，會被儲存到資料庫（DB）裡， 成為我們未來用來串連、比對數據的重要基礎。
...
- **第 1 階：觀察數據** AI 系統呈現事實：越南提供新補貼、55 吋面板報價下跌 5%、競爭對手 S 公司在越南市佔率成長 3%、我方銷售額持平。

- **第 2 階：選擇性接收** 他的注意力，立刻被「對手市佔率成長」和「我方銷售額持平」的負面資訊給吸住，卻忽略了「面板報價下跌」的利好消息。

- **第 3-7 階：快速跳躍** 他迅速將資訊解讀為「我們正失去競爭力」，並跳躍式地假設「對手一定是在降價」，最終形成「必須立即跟進降價」的結論與信念，要求團隊投入價格戰。
...
AI 不再只是閱讀文字， 它學會了理解文字背後的結構與關聯。

它能從一句話中，辨識出誰是主角、誰是配角， 以及他們之間發生了什麼事。
