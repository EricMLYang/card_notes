...
`000_MyContext/` 資料夾包含個人化配置檔案，控制 AI Skills 的行為：

- `capture_profile.md`：閱讀偏好與關注方向，由 `scout-news` 和 `update-profile` skills 使用
- `break_cards_profile.md`：卡片拆解偏好，由 `break-cards` skill 使用
- `004_0Question/`：問題意識與探索方向
- `人名清單.md`：常用人名對照表

這些檔案讓 AI 能根據用戶偏好自動調整行為，實現真正的個人化自動化。
